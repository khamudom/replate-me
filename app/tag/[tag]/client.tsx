/**
 * Client-side component for displaying recipes filtered by a specific tag.
 * This component fetches recipes from Supabase using the get_recipes_by_tag RPC function
 * and displays them in a responsive grid layout with a fixed header showing the tag name.
...
export default function TagPageClient({ tag }: TagPageClientProps) {
  ...
  useEffect(() => {
    const fetchRecipesByTag = async () => {
      try {
        const { data, error } = await supabase.rpc("get_recipes_by_tag", {
          tag_name: tag,
        });
      ...
      <Box sx={{ paddingTop: "70px" }}>
        <Grid container spacing={3} sx={{ marginBottom: "2rem" }}>
          ...
          {recipes.length === 0 && (
            <Grid item xs={12}>
              <Typography>
                No recipes found with the tag &quot;{tag}&quot;.
              </Typography>
            </Grid>
          )}
        </Grid>
